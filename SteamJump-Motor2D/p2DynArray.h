...
public:
	// Constructors -------------
	p2DynArray() : mem_capacity(0), num_elements(0), data(NULL) { Alloc(DYN_ARRAY_BLOCK_SIZE); }
	p2DynArray(unsigned int capacity) : mem_capacity(0), num_elements(0), data(NULL) { Alloc(capacity); }

	// Destructor ---------------
	~p2DynArray() { delete[] data; }
	...
	{
		if (num_elements + array.num_elements > mem_capacity)Alloc(num_elements + array.num_elements);

		for (uint i = 0; i< array.num_elements >mem_capacity)
	...
	{
		if (num_elements >= mem_capacity)Alloc(mem_capacity + DYN_ARRAY_BLOCK_SIZE);
	...
	{
		if (position > num_elements)return false;
		...
		if (num_elements + 1 > mem_capacity)Alloc(mem_capacity + DYN_ARRAY_BLOCK_SIZE);
	...
	bool Insert(const p2DynArray<VALUE>& array, unsigned int position)
	{
		...
		if (num_elements + array.num_elements > mem_capacity)Alloc(num_elements + array.num_elements + 1);
	...
	VALUE* At(unsigned int index)
	{
		...
		if (index < num_elements)return result = &data[index];
		return result;
	}
